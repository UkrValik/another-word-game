...
import {
  GuessAttemptLetters,
  GuessAttemptLettersMethods,
} from "./guess-attempt-letters";
...
export const GameScreen = ({ route }: Props) => {
  ...
  const attemptsArray: (Attempt | number)[] = [];

  for (let i = 0; i < game.attemptsCount; ++i) {
        ...
        <View style={styles.attemptsWrapper}>
          {attemptsArray.map((attempt, index) => {
            const activeAttempt =
              ...
              (typeof attemptsArray[index - 1] === "object" &&
                typeof attemptsArray[index] === "number");
            return (
              <GuessAttemptLetters
                key={JSON.stringify(attempt)}
                letterCount={game.secretWord.length}
                attempt={attempt}
